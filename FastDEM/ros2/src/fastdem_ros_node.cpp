...
class MappingNode : public rclcpp::Node {
 ...
 private:
  ...
  void publishLocalView() {
    ...
    const bool want_gridmap = pub_gridmap_->get_subscription_count() > 0;
  ...
  void processPendingScans() {
    ...
    while (pending_scans_.size() > max_pending_scans_) {
      const auto& stale = pending_scans_.front();
      spdlog::warn(
          "Dropping stale scan while waiting for TF readiness. stamp={}.{:09d}",
          stale->header.stamp.sec, stale->header.stamp.nanosec);
      pending_scans_.pop_front();
    }
  }
  ...
  std::deque<sensor_msgs::msg::PointCloud2::SharedPtr> pending_scans_;
  const std::size_t max_pending_scans_ = 20;
